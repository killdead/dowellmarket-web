...
function menu_builder_site_menu_register($hook, $type, $return, $params) {
	...
	if(!elgg_is_logged_in()){
		$options["wheres"] = array("e.access_id IN (" . ACCESS_PUBLIC . ", " .MENU_BUILDER_ACCESS_NOT_LOGGED.",". MENU_BUILDER_ACCESS_LOGGED_OUT . ")");
	}
	
	if(elgg_is_logged_in() && !elgg_is_admin_logged_in()){
		$options["wheres"] = array("e.access_id NOT IN (" . MENU_BUILDER_ACCESS_NOT_LOGGED . ")");
	}
	...
	if(elgg_in_context("menu_builder")){
		$result = array(
			...
			MENU_BUILDER_ACCESS_LOGGED_OUT => elgg_echo("LOGGED_OUT"), 
				MENU_BUILDER_ACCESS_NOT_LOGGED => elgg_echo("PUBLIC NOT LOGGED"),
