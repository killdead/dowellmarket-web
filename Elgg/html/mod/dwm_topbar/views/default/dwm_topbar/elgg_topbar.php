...
<?php
     ...
     if (elgg_is_logged_in() || logoutbar_enabled()) {
     	
     	elgg_log("TOPBAR IN IF", "DEBUG");
?>

<div id="elgg_topbar" <?php if (!elgg_is_logged_in()){?> class="elgg_topbar_loggedout"<?php }?>>
<?php if (elgg_is_logged_in()){
		$joinicontools = elgg_get_plugin_setting('joinicontools','vazco_topbar');
		$joinsettings = elgg_get_plugin_setting('joinsettings','vazco_topbar');
		$elgglogo = elgg_get_plugin_setting('elgglogo','vazco_topbar');
		$homebutton = elgg_get_plugin_setting('homebutton','vazco_topbar');
	...
	<?php 
	if ($homebutton != 'no'){
		elgg_log("on affiche HOME", "DEBUG");
	...
	?>
		<div class="toolbarimages">
			...
			if ($joinicontools == 'no'){?>
				...
				<?php 
				if (elgg_is_active_plugin('vazco_avatar') && vazco_avatar::hasFlashAvatar($_SESSION['user'])){
