...
//	elgg_register_plugin_hook_handler('register', 'menu:owner_block', 'market_owner_block_menu');
	...
	elgg_register_plugin_hook_handler('register', 'menu:entity', 'market_entitymenu', 1000);
...
/*
 * 
 * Remove Access item from from market item menu
 * 
 * 
 */
function market_entitymenu($hook, $type, $return, $params) {
	...
	foreach ($return as $key => $item) {
		...
		if ($item->getName() == 'edit') {
		 $return[$key]->setText(elgg_view_icon('eye'));
		}
